...
- un deuxième essai echoue et la encore avec le retour de ansible on peut se rendre compte que c'est le fait de vouloir utiliser un password sur la connection ssh en ligne de commande n'est pas autorisé. ssh demande une saisie interactive lorsqu'on l'utilise en ligne de commande et en mettant les valeur ansible_user ansible_ssh_pass c'est comme si on utilisait le mot de passe directement en ligne sans saisie interactive par défaut la connexion ssh ne le permet pas il faut donc installer le paquet sshpass qui va jouer le rôle d'intermédiaire ici pour passer nos password. En d'autre mots en passant le paramètre --ask-pass (-k) pour saisir le mot de passe on à pas forcément besoin de sshpass mais vu que on a entré notre mot de passe en dur ce paquet est nécessaire.
